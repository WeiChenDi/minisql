...
{
	int recordSize = 0;
	if ((int)tbl.attri.size() > MAXATTRINUM)
	{
		cout << "Failure Message:: Too many attributes!" << endl;
		cout << "Failure Message:: Create table failed!" << endl;
		cout << endl;
		return;
	}
	for (int i = 0; i < tbl.attri.size(); i++)
	{
		recordSize += getTypeSize(tbl.attri[i].type);
		if (tbl.attri[i].type != INT && tbl.attri[i].type != FLOAT)
		{
			if (getTypeSize(tbl.attri[i].type) > MAXCHARSIZE)
			{
				cout << "Failure Message:: Too big char!" << endl;
				cout << "Failure Message:: Create table failed!" << endl;
				cout << endl;
				return;
			}
			else if (getTypeSize(tbl.attri[i].type) < MINCHARSIZE)
			{
				cout << "Failure Message:: Too small char!" << endl;
				cout << "Failure Message:: Create table failed!" << endl;
				cout << endl;
				return;
			}
	...
	int primaryPosi = tbl.primaryKey;
	string primaryAttri = tbl.attri[tbl.primaryKey].name;
	string indexName = primaryAttri + "Index";
	CreateIndex((char*)indexName.c_str(), (char*)tbl.name.c_str(), (char*)primaryAttri.c_str());
}
...
void API::InsertRecord(string tableName, vector<string> recordContent)
{
	...
	it = tmpTable->attri.begin();
	for (; it != tmpTable->attri.end(); it++)				//检查是否存在索引
	{
		if (tmpTable->attri[i].type != INT and tmpTable->attri[i].type != FLOAT)
		{
			if (getTypeSize(tmpTable->attri[i].type) < recordContent[i].length())
			{
				cout << "Failure Message:: Wrong insert content" << endl;
				cout << "Failure Message:: The char is too big!" << endl;
				cout << "Failure Message:: Insert failed!" << endl;
				cout << endl;
				return;
			}
		}
		else if (tmpTable->attri[i].type == INT)
		{
			if (recordContent[i].length() > 10)
			{
				cout << "Failure Message:: Wrong insert content" << endl;
				cout << "Failure Message:: The int is too big!" << endl;
				cout << "Failure Message:: Insert failed!" << endl;
				cout << endl;
				return;
			}
	...
	i = 0;
	for (it = tmpTable->attri.begin(); it!=tmpTable->attri.end(); it++)			//将record中的所有有index内容插入到B+树中
	{
	...
	printf("Success Message:: Insert record into table %s successfully\n\n", tableName.c_str());
	bm.SetDirty(btmp);
	btmp->RLUcount++;
	delete[] charContent;
}
...
{
	cout << endl << "【" << tbl->name << "】" << endl;
	int lineLen = 0;
	for (int i = 0; i < tbl->attri.size(); i++)
	{
		int type = tbl->attri[i].type;
		switch (type)
		{
		case INT: 
			lineLen += 11;
	...
	for (int i = 0; i < tbl->attri.size(); i++)
	{
		...
		case FLOAT: len = 10;
			break;
		default: len = getTypeSize(type)+2;
			break;
		}

		cout << "|" << left << setw(len) << tbl->attri[i].name;
	}
	cout << "|";
	lineLen++;
	cout << endl;
...
void API::tableDrop(string tableName)
{
	...
	for (int i = 0; i < tmpTable->attri.size(); i++)
	{
		if (tmpTable->attri[i].index != "")
		{
			IndexDrop(tmpTable->attri[i].index);
		}
	}
	bm.DropTable(tableName);
	rm->DropTable(tableName);
	cm->DropTable(tableName);
	cout << "Success Message:: Table " << tableName << "drop successfully!" << endl;
	cout << endl;
}
...
void API::deleteAllRecord(string tableName)
{
	...
	cout << "Success Message:: All records in the " << tableName << " table are deleted successfully!" << endl;
	cout << endl;
}
...
void API::DeleteConditionRecord(string tableName, Condition conditionValue)
{
	Table* tmpTable = NULL;
	int i = 0;
	for (i = 0; i < cm->tableList.size(); i++)				//找到表
	{
		if (cm->tableList[i].name == tableName)
		{
			tmpTable = &cm->tableList[i];
	...
	set<OffSetNumber> tmpSet;			//返回的blockId集合
	string conditionAttri = conditionValue.attriName;
	i = 0;
	for (i = 0; i < tmpTable->attri.size(); i++)		//遍历所有attribute
	{
		if (tmpTable->attri[i].name == conditionAttri && tmpTable->attri[i].index != "")			//在table中找到条件的attribute并且存在index
		{
			...
			set<OffSetNumber>::iterator it = tmpSet.begin();
			for (; it != tmpSet.end(); it++)
			{
				int recordDeleted = rm->DeleteConditionRecordByBlock(tmpTable, *it, conditionValue);
				count += recordDeleted;
			}
			cm->DeleteConditionRecord(tableName, count);
			cout << "Success Message:: " << count << " records are deleted by index" << endl;
			cout << endl;
			return;
		}
	}
...
void API::CreateIndex(char* indexName, char*  tableName, char*  attriName)
{
	...
	if (rm->CreateIndex(indexName, tableName, attriName, attriType, 5))		//建立索引文件夹
	{
		...
		im.CreateIndex(indexName, attriType, 25);				//建立索引，即B+树头结点和索引结构
		cout << "Success Message:: Create index " << indexName << " successfully!" << endl;
		cout << endl;
...
void API::IndexDrop(string indexName)
{
	...
	cout << "Success Message:: Drop index " << indexName << " successfully" << endl;
	cout << endl;
}
