...
TreeNode<KeyType>* TreeNode<KeyType>::splite(KeyType &key)
{
	...
	if (!this->isLeaf)
	{
		...
		{
			nNode->value[i] = this->value[i + minnum + 1];
			nNode->child[i] = this->child[i + minnum + 1];
			nNode->child[i]->parent = nNode;
			//this->value[i + minnum] = KeyType();
			//this->child[i + minnum] = NULL;
		}
		nNode->count = degree - minnum - 1;
		nNode->child[degree - minnum - 1] = this->child[degree];
		nNode->child[degree - minnum - 1]->parent = nNode;
		this->count = minnum;
	}
	else
	{
...
bool BPlusTree<KeyType>::AdjustAfterInsert(TreeNode<KeyType> *t)
{
	...
	for (int i = t->parent->count; i > ind; i--)
	{
		nNode->parent->child[i] = nNode->parent->child[i - 1];
	}
...
bool BPlusTree<KeyType>::adjustAfterDelete(TreeNode<KeyType> *pNode)
{
	...
	else
	{
		...
		if (pNode->isLeaf)
		{
			...
			if ((parent->child[0] != pNode) && (index + 1 == parent->count) || index == parent->count) //choose the left brother to merge or replace
			{
		...
		{
			int index = 0;
			TreeNode<KeyType> *last = pNode->child[0];
			while (last->child[0] != NULL)
			{
				last = last->child[0];
			}
			parent->search(last->value[0], index);
			if ((parent->child[0] != pNode) && (index + 1 == parent->count) || index == parent->count) // choose the left brother to merge or replace
			{
				brother = parent->child[index];
			...
			else // choose the right brother
			{
				...
				if (brother->count > minmumKey - 1)// choose the most left key and child to add to the right hand of the pnode
				{
					...
					if (pNode->count == 0)
					{
						TreeNode<KeyType> *last = brother->child[0];
						while (last->child[0] != NULL)
						{
							last = last->child[0];
						}
						pNode->value[0] = last->value[0];
					}
