...
int BufferManager::WriteBackToDisk(string fileName, int writeBackBlock, int blockType)
{
	...
	{
		tmpint1.num = btmp->using_Size;
		//cout << "usingsize::" << tmpint1.num << endl;
		char* address = NULL;		//更新usingsize
		address = btmp->content + 4;
		memcpy(address, tmpint1.byte, 4);
		char* tmpContent = new char[4096];
		...
		//memcpy(tmpint1.byte, btmp->content, 4);
		//cout << "blockID::" << tmpint1.byte << endl;
		//memcpy(tmpint1.byte, btmp->content + 4, 4);
		//cout << "usingsize::" << tmpint1.num << endl;
		//int recordNum = (btmp->using_Size - btmp->head_Size) / 8;
		//cout << "recordNum::" << recordNum << endl;
		//system("pause");
		//char* t = btmp->content + btmp->head_Size;
		//for (int i = 0; i < recordNum; i++)
		//{
		//	memcpy(tmpint1.byte, t, 4);
		//	cout << "BookNo::" << tmpint1.num << endl;
		//	t += 4;
		//	memcpy(tmpint1.byte, t, 4);
		//	cout << "BlockID::" << tmpint1.num << endl;
		//	t += 4;
		//	system("pause");
		//}
		...
		int fileHeadSize;
		fstream fout;
		fout.open(file, ios::binary | ios::in | ios::out);
